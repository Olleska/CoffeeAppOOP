...
public class DrinkController
{
    ...
    private IElement BuildRecipe()
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("И1. Вода");
            Console.WriteLine("И2. Кофейные зерна");
            Console.WriteLine("И3. Молоко");
            Console.WriteLine("И4. Сироп");
            Console.WriteLine("И5. Лед");
            Console.WriteLine("И6. Сок");
            Console.WriteLine("--- Действия (Используют ингредиенты или действуют сами по себе) ---");
            Console.WriteLine("Д1. Добавить ингредиент (использует уже определенный)");
            Console.WriteLine("Д2. Вскипятить (использует уже определенный)");
            Console.WriteLine("Д3. Перемолоть (использует уже определенный)");
            Console.WriteLine("Д4. Перемешать (не требует ингредиентов)");
            Console.WriteLine("Д5. Пролить (требует два уже определенных ингредиента)");
            Console.WriteLine("Д6. Взбить (использует уже определенный)");
            Console.WriteLine("0. Завершить рецепт");
            Console.Write("Ваш выбор (И1, Д1, 0, и т.д.): ");
            ...
            switch (input)
            {
                case "И1": 
                    ...
                    break;
                case "И2": 
                    chosenIngredientForAction = GetIngredientDetails<CoffeeBeans>("Введите сорт кофе: ", "Введите вес кофейных зерен (г): ");
                    if (chosenIngredientForAction != null)
                        ingredientsAddedToCurrentRecipe.Add(chosenIngredientForAction);
                    break;
                case "И3": 
                    chosenIngredientForAction = GetIngredientDetails<Milk>("Введите тип молока (например, 'коровье', 'соевое'): ", "Введите вес молока (г): ");
                    if (chosenIngredientForAction != null)
                        ingredientsAddedToCurrentRecipe.Add(chosenIngredientForAction);
                    break;
                case "И4": 
                    chosenIngredientForAction = GetIngredientDetails<Syrop>("Введите тип сиропа (например, 'карамельный'): ",
                ...
                case "И5": 
                    ...
                    break;
                case "И6": 
                    chosenIngredientForAction = GetIngredientDetails<Juice>("Введите вкус сока (например, 'апельсиновый'): ",
                        "Введите вес сока (г): ");
                    if (chosenIngredientForAction != null)
                        ingredientsAddedToCurrentRecipe.Add(chosenIngredientForAction);
                    break;
                ...
                case "Д2": 
                    ...
                    break;
                case "Д3": 
                    chosenIngredientForAction = SelectIngredientFromList(ingredientsAddedToCurrentRecipe, "перемалывания");
                    if (chosenIngredientForAction == null)
                    {
                        Console.WriteLine("Не выбран ингредиент.");
                        break;
                ...
                case "Д4": 
                    ...
                    break;
                case "Д5": 
                    chosenLiquidForPour = SelectIngredientFromList(ingredientsAddedToCurrentRecipe, "проливания (жидкость)");
                    ...
                    chosenMediumForPour = SelectIngredientFromList(ingredientsAddedToCurrentRecipe, "проливания (среда)");
    ...
    private Ingredient.Ingredient SelectIngredientFromList(List<Ingredient.Ingredient> availableIngredients, string actionContext)
    {
    ...
    private T GetIngredientDetails<T>(string prompt1, string prompt2 = null) where T : Ingredient.Ingredient
    {
        ...
        try
        {
            ...
            else if (typeof(T) == typeof(CoffeeBeans))
            {
                if (decimal.TryParse(param2, out decimal weight))
                {
                    createdIngredient = new CoffeeBeans(param1, weight);
                }
            }
            ...
            else if (typeof(T) == typeof(Juice))
            {
                if (decimal.TryParse(param2, out decimal weight))
                {
                    createdIngredient = new Juice(param1, weight);
                }
            }
